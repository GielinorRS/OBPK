...
public class Halloween extends PacketInteraction {
    ...
    public boolean handleButtonInteraction(Player player, int button) {
        if (button == 73307) {
            player.optionsTitled("Exchange your 5,000 OBS1 tokens for a reward?", "Yes", "No", () -> {
                ...
                if (player.inventory().contains(new Item(HWEEN_TOKENS, 5000))) {
                    ...
                    player.message(Color.RED.wrap("You do not have enough OBS1 tokens to roll for a reward."));
                }
            });
            return true;
        }
        if (button == 73310) {
            player.getEventRewards().reset("OBS1");
            return true;
        }
        return false;
    }
