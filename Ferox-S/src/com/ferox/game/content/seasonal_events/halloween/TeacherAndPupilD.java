...
public class TeacherAndPupilD extends Dialogue {
    ...
    protected void select(int option) {
        ...
        if (isPhase(6)) {
            if (option == 1) {
                send(DialogueType.ITEM_STATEMENT, new Item(PUMPKIN_LANTERN), "", "The teacher hands you a Pumpkin Lantern and a Killer's knife to collect", "candies.");
                setPhase(13);
                player.inventory().addOrDrop(new Item(PUMPKIN_LANTERN));
                player.inventory().addOrDrop(new Item(KILLERS_KNIFE_21059));
        ...
        if (isPhase(12)) {
            if (option == 1) {
                ...
                player.message(Color.PURPLE.wrap("The teacher gave you "+ Utils.formatNumber(count)+" OBS1 mystery "+plural+" in return."));
                stop();
            }
            if (option == 2) {
                stop();
