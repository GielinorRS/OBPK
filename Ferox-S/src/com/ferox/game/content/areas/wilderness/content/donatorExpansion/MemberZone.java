package com.ferox.game.content.areas.wilderness.content.donatorExpansion;
...
public class MemberZone extends PacketInteraction {
    ...
    public static boolean canAttack(Mob attacker, Mob target) {
        if (attacker.isPlayer() && target.isNpc()) {
            ...
            Npc npc = (Npc) target;
            var member = player.getMemberRights().isRegularMemberOrGreater(player);
            var super_member = player.getMemberRights().isSuperMemberOrGreater(player);
            ...
            var legendary_member = player.getMemberRights().isLegendaryMemberOrGreater(player);
            var vip_member = player.getMemberRights().isVIPOrGreater(player);
            var sponsor_member = player.getMemberRights().isSponsorOrGreater(player);
    ...
    public boolean handleObjectInteraction(Player player, GameObject obj, int option) {
        if (option == 1) {
            ...
            if (obj.getId() == VIP_CASTLE_PORTAL) {
                if (player.tile().memberZone())
                if(player.getMemberRights().isVIPOrGreater(player)) {
                    Teleports.basicTeleport(player, new Tile(2600, 3875));
                } else {
                }
                return true;
            }
                ...
                player.getDialogueManager().start(new Dialogue() {
                    ...
                    protected void select(int option) {
                        if (option == 1) {
                            ...
                            Teleports.basicTeleport(player, tile);
                            player.message("You have been teleported to the NightMare Boss.");
                        } else if (option == 2) {
                            stop();
                        }
                    }
