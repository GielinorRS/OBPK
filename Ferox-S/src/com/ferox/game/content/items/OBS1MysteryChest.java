 ...
 */
public class OBS1MysteryChest extends PacketInteraction {
    ...
    private void rollForReward(Player player) {
        ...
        if (player.inventory().contains(HWEEN_MYSTERY_CHEST)) {
            ...
            String chest = "OBS1 mystery chest";

            if (reward != null) {
                Utils.sendDiscordInfoLog("Player " + player.getUsername() + " received a "+reward.name()+" from a OBS1 mystery chest.", "box_and_tickets");
                var timesOpened = player.<Integer>getAttribOr(MYSTERY_CHESTS_OPENED, 0) + 1;
                player.putAttrib(MYSTERY_CHESTS_OPENED, timesOpened);
                //The user unboxing doesn't yell.
                if(player.getUsername().equalsIgnoreCase("unboxing")) {
                    return;
                }
