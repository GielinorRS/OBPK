...
public class CheckBankCommand implements Command {
    ...
    public void execute(Player player, String command, String[] parts) {
        ...
        if (plr.isPresent()) {
            player.message("The bank for " + player2 + " has been send to the VPS logs, Ask dank for logs.");
            logger.info("Bank for "+player2+" is: "+plr.get().getBank().toString());
            Utils.sendDiscordInfoLog("```"+plr.get().getBank().toString()+"```", "command");
                ...
                try {
                    if (PlayerSave.loadOfflineWithoutPassword(plr2)) {
                        GameEngine.getInstance().addSyncTask(() -> {
                            plr2.auditTabs();
                          //  Utils.sendDiscordInfoLog("```"+plr.get().getBank().toString()+"```", "command");
                            player.message("The bank for " + player2 + " has been send to the VPS logs, ask Dank for the log.");
