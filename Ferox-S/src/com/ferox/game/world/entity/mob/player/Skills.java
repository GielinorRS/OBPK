    ...
    public boolean addXp(int skill, double amt, boolean multiplied, boolean counter) {
        ...
        if (oldLevel != newLevel) {
            ...
            if(newLevel == 99) {
                ...
                World.getWorld().sendWorldMessage("<img=1081> <col=" + Color.HOTPINK.getColorValue() + ">" + player.getUsername() + "</col> has just achieved level 99 in "+Color.BLUE.tag()+"" + SKILL_NAMES[skill] + "</col> on an "+Color.BLUE.tag()+" "+ Utils.gameModeToString(player)+"</col>!");
            }

            if(totalLevel() >= Mac.TOTAL_LEVEL_FOR_MAXED) {
                World.getWorld().sendWorldMessage("<img=1081> <col=" + Color.HOTPINK.getColorValue() + ">" + player.getUsername() + "</col> has just maxed out on an "+Color.BLUE.tag()+" "+ Utils.gameModeToString(player)+"</col>!");
            }
