...
import com.ferox.game.content.seasonal_events.christmas.SnowMonsterSpawnTask;
...
import static com.ferox.game.content.seasonal_events.christmas.SnowMonsterSpawnTask.*;
            ...
            Chain.bound(null).runFn(npc.combatInfo() != null ? npc.combatInfo().deathlen : 5, () -> {
                if (killer != null) {
                    ...
                    if (table != null && (npc.id() != KALPHITE_QUEEN_6500 && npc.id() != RUNITE_GOLEM && !npc.isWorldBoss() && npc.id() != THE_NIGHTMARE_9430)) {
                        ...
                        boolean doubleDropsLampActive = (Integer) killer.getAttribOr(DOUBLE_DROP_LAMP_TICKS, 0) > 0 && Math.random() <= 0.04;
                        boolean founderImp = killer.pet() != null && killer.pet().def().name.equalsIgnoreCase("Founder imp pet") && Math.random() <= 0.10;
                            ...
                            table.getGuaranteedDrops().forEach(tableItem -> {
                                if (killer.inventory().contains(13116)) {
                                    ...
                                    if (tableItem.min > 0) {
                                        ...
                                        if (killer.nifflerPetOut() && killer.nifflerCanStore(npc) && dropped.getValue() > 0) {
                                        ...
                                        if (killer.nifflerPetOut() && killer.nifflerCanStore(npc) && tableItem.convert().getValue() > 0) {
                        ...
                        Item item = Item.of(CustomItemIdentifiers.DRAGON_HUNTER_CROSSBOW_T);
                        if (killer.getSlayerRewards().getUnlocks().containsKey(SlayerConstants.DOUBLE_DROP_CHANCE) && npc.combatInfo().boss && Math.random() <= .25) {
                            dropRolls += 1;
                            killer.message("@blu@<shad=1>The Double drops perk grants you a second drop!");
                        }
                        //this code will search for droped item if he found dragon hunter crossbow t droped from elvarg will add it to collection log sweet





                        if(item == item && npc.id() == ELVARG_HARD){//updatet1
                            OTHER.log(killer, NpcIdentifiers.ELVARG_HARD, new Item(CustomItemIdentifiers.DRAGON_HUNTER_CROSSBOW_T));
                        ...
                        for (int i = 0; i < dropRolls; i++) {
                            ...
                            if (reward != null) {
                                if (doubleDropsLampActive || founderImp) {//they already here lamp dont work? im told no never tried it myself one sec
                                    //Drop the item to the ground instead of editing the item instance
                                    GroundItem doubleDrop = new GroundItem(reward, tile, killer);
                                    if (killer.nifflerPetOut() && killer.nifflerCanStore(npc) && reward.getValue() > 0) {
                                        killer.nifflerStore(doubleDrop.getItem());
                                    } else {
                                        GroundItemHandler.createGroundItem(doubleDrop);
                                    }
                                    killer.message("@blu@<shad=1>The double drop effect doubled your drop.");
                                }
                                ...
                                if (reward.getId() == GIANT_PRESENT) {
                                    if (!killer.getSackOfPresents().pickup(reward, npc)) {
                                        GroundItemHandler.createGroundItem(new GroundItem(reward, tile, killer));
                                    }
                                } else if (killer.nifflerPetOut() && killer.nifflerCanStore(npc) && reward.getValue() > 0) {
    ...
    public static Optional<Pet> checkForPet(Player killer, ScalarLootTable table) {
        ...
        if (pet.isPresent()) {
            ...
            switch (pet.get()) {
                ...
                case BABY_LAVA_DRAGON -> OTHER.log (killer, LAVA_DRAGON, new Item(Pet.BABY_LAVA_DRAGON.item));//updatet3
                case DERANGED_ARCHAEOLOGIST -> OTHER.log(killer, NpcIdentifiers.DERANGED_ARCHAEOLOGIST, new Item(Pet.DERANGED_ARCHAEOLOGIST.item));
                //end
