package com.Ardevon.scene;

import com.Ardevon.Client;
import com.Ardevon.ClientConstants;
import com.Ardevon.cache.def.FloDefinition;
...
import com.Ardevon.util.ChunkUtil;
...
public final class Region {
                        ...
                        if (y >= 1 && y < region_size_y - 1 && (!low_detail || (tile_flags[0][x][y] & 2) != 0 || (tile_flags[plane][x][y] & 0x10) == 0 && get_visibility(y, plane, x) == this.plane)) {
                            ...
                            if (underlay_id > 0 || overlay_id > 0) {
                                ...
                                if (overlay_id == 0) {
                                    ...
                                    int overlay_hsl;
                                    int minimap_color;
                                    if (overlay_texture_id == 51) {
                                        overlay_texture_id = 3;
                                    }
    ...
    private void render(int y, SceneGraph scene, CollisionMap map, int type, int plane, int x, int object_id, int orientation) {
        ...
        if (orientation == 1 || orientation == 3) {
            obj_size_offset_a = def.objectSizeY;
            obj_size_offset_b = def.objectSizeX;
        } else {
            obj_size_offset_a = def.objectSizeX;
            obj_size_offset_b = def.objectSizeY;
        }
        ...
        int v_sw = vertex_heights[plane %= 4][obj_x_factor_a][obj_y_factor_a];
        ...
        if (type == 22) {
            if (low_detail && def.isInteractive == 0 && !def.obstructsGround) {
                return;
            }
            ...
            if (def.solid && def.isInteractive == 1 && map != null)
        ...
        if (type == 10 || type == 11) {
            ...
            if (scene_objects != null) {
                ...
                int size_offset_b;
                if (orientation == 1 || orientation == 3) {
                    size_offset_a = def.objectSizeY;
                    size_offset_b = def.objectSizeX;
                } else {
                    size_offset_a = def.objectSizeX;
                    size_offset_b = def.objectSizeY;
                }
                if (scene.add_entity(key, avg, size_offset_b, ((Renderable) (scene_objects)), size_offset_a, plane, obj_rotation, y, x) && def.castsShadow) {
                    Model model;
            ...
            if (def.solid && map != null)
                map.mark_interactive_obj(def.impenetrable, def.objectSizeX, def.objectSizeY, x, y, orientation);
        ...
        if (type == 0) {
            ...
            if (def.solid && map != null)
                map.mark_wall(y, orientation, x, type, def.impenetrable);

            if (def.decorDisplacement != 16)
                scene.offset_wall_decor(y, def.decorDisplacement, x, plane);
            ...
            do {
                ...
                do {
                    ...
                    if (plane == sub_plane && region_x >= width && region_x < width + 8 && region_y >= height && region_y < height + 8) {
                        ObjectDefinition def = ObjectDefinition.get(object_id);
                        int x = x_offset + ChunkUtil.get_rotated_landscape_x(rotation, def.objectSizeY, region_x & 7, region_y & 7, def.objectSizeX);
                        int y = y_offset + ChunkUtil.get_rotated_landscape_y(region_y & 7, def.objectSizeY, rotation, def.objectSizeX, region_x & 7);
    ...
    public static void place(SceneGraph scene, int orientation, int y, int type, int plane, CollisionMap collision, int vertex_heights[][][], int x, int object_id, int z) {
        ...
        if (type == 22) {
            ...
            if (def.solid && def.isInteractive == 1)
        ...
        if (type == 10 || type == 11) {
            ...
            if (def.solid)
                collision.mark_interactive_obj(def.impenetrable, def.objectSizeX, def.objectSizeY, x, y, orientation);
        ...
        if (type == 0) {
            ...
            if (def.solid)
                collision.mark_wall(y, orientation, x, type, def.impenetrable);
        ...
        do {
            ...
            do {
                if (read_second) {
                    ...
                    if (x > 0 && y > 0 && x < 103 && y < 103) {
                        ObjectDefinition def = ObjectDefinition.get(object_id);
                        if (obj_type != 22 || !low_detail || def.isInteractive == 1 || def.obstructsGround) {
                            cached &= def.cached();
                            read_second = true;
                        }
                    }
            ...
            do {
                ...
                do {
                    ...
                    if (x > 0 && y > 0 && x < 103 && y < 103 && plane >= 0 && plane < 4) {
                        ...
                        CollisionMap collision = null;
                        if (marking_plane >= 0)//added for higher revisions
                            collision = map[marking_plane];
