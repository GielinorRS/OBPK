...
public class ItemSpawnCommand implements Command {
    ...
    public void execute(Player player, String command, String[] parts) {
       ...
       Item item = new Item(id);
        ...
        if (Item.valid(item) && (player.getPlayerRights().isDeveloperOrGreater(player) || (player.getPlayerRights().isEventManager(player) || GameServer.properties().test || item.definition(World.getWorld()).pvpAllowed))) {
    ...
    public boolean canUse(Player player) {
        if (!GameServer.properties().pvpMode) {
            return (player.getPlayerRights().isDeveloperOrGreater(player) || player.getPlayerRights().isEventManager(player));
        }
        return true;
    }
}
